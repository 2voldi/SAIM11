/**
 * Goat Bot Cleaned (no jsonlint)
 * Based on official Goat-Bot-V2 by NTKhang
 */
...
function validJSON(pathDir) {
  try {
    ...
    throw new Error(`JSON parse error in "${pathDir}": ${err.message}`);
  }
}
...
if (config.autoRestart) {
  ...
  if (!isNaN(time) && time > 0) {
    utils.log.info("AUTO RESTART", `Bot will restart after ${utils.convertTime(time, true)}`);
    setTimeout(() => {
      utils.log.info("AUTO RESTART", "Restarting...");
      process.exit(2);
    }, time);
  } else if (typeof time === "string" && time.match(/^((((\d+,)+\d+|(\d+(\/|-|#)\d+)|\d+L?|\*(\/\d+)?|L(-\d+)?|\?|[A-Z]{3}(-[A-Z]{3})?) ?){5,7})$/gmi)) {
    utils.log.info("AUTO RESTART", `Bot will restart by cron: ${time}`);
...
(async () => {
  ...
  if (compareVersion(version, currentVersion) === 1)
    utils.log.master("NEW VERSION", `Update available: ${version} (current: ${currentVersion})`);
